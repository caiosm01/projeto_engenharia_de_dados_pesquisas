...
class DataFrameProcessor:
    ...
    """
    def __init__(self, Mysql=None, df=None, table_name=None, id_projeto=None, id_questionario=None, column_mappings=False):
        """
        ...
        self.id_questionario = id_questionario
    ...
    def fetch_base_data(self):
        ...
        try:
            if self.table_name == 'mydb.pesquisas':
                return self.Mysql.fetch_all("select * from mydb.pesquisas where MONTH(`Data_Inicio`) = month(current_date);")
            elif self.table_name == 'banco_sac2.satisfacao' or self.table_name == 'banco_sac2.medicao':
                return self.Mysql.fetch_all(f"""select * from {self.table_name} S 
                left join mydb.pesquisas P ON S.`Nro. Identificação`=P.idPesquisa 
                where year(P.`Data_Inicio`) = year(current_date()) and month(P.`Data_Inicio`)=month(current_date());""")
            else:
                pass
        except:
            pass
    ...
    def convert_to_date_or_datetime(self, col):
        ...
        try:
            ...
            #temp_col = pd.to_datetime(self.df[col], format='%d %m de %Y')
    ...
    def process_data_2(self, sem_meta_dados=True, sem_duplicadas=True):
        ...
        if sem_meta_dados:
            ...
            self.df['idQuestionario'] = self.id_questionario
            self.df = self.df[['idProjeto', 'idQuestionario', 'Pesquisador', 'Nro. Identificação', 'Data Início', 'Data Fim', 'Latitude', 'Longitude']]
        # Retorna verdadeiro se houver linhas para processar
        self.df = self.df[~self.df['Nro. Identificação'].astype(str).isin(self.base['idPesquisa'].astype(str))]
        #print(self.df['Nro. Identificação'])
    ...
    def execute_insert_queries(self, vs=None):
        """
        ...
        if vs:
            ...
            insert_queries = self.generate_insert_queries(self.table_name)
        #print(len(insert_queries))
        for query in insert_queries:
            #print(query[1])  # Imprime a query
            result = self.Mysql.execute_query(query[0], query[1])  # Executa a query
            if result == "dado inserido no banco":
                dados_inseridos += 1
                continue
            #print(result)
            ...
            if result == "duplicado":
                ...
                return [result, query[1], len(insert_queries), dados_inseridos]
        return f"dados inseridos: {dados_inseridos} de {len(insert_queries)}"
